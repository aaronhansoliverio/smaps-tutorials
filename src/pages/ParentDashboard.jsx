...
import { parentModules, getParentTotalVideos } from '../data/courseData'
...
export default function ParentDashboard() {
  ...
  const [openModules, setOpenModules] = useState(['p1'])
  const [completedVideos, setCompletedVideos] = useState(() => {
    const saved = localStorage.getItem('smaps_parent_completed_videos')
    return saved ? JSON.parse(saved) : []
  })
  const [bannerError, setBannerError] = useState(false)

  const totalVideos = getParentTotalVideos()
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <Navbar title="SMAPS-SIS Parent Tutorials" showBack backTo="/" backLabel="Home" />
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <h1 className="text-xl md:text-3xl font-extrabold leading-tight">
                SMAPS-SIS Parent Tutorials
              </h1>
              <p className="text-red-200 mt-2 text-sm leading-relaxed max-w-2xl">
                Complete video tutorial series for parents. Learn how to access the Parent Portal,
                monitor your child's grades and attendance, manage finances, and communicate with
                the school — at your own pace.
              </p>
              <div className="flex flex-wrap gap-3 mt-3 text-xs text-red-300">
                <span>📹 {totalVideos} videos</span>
                <span>📦 {parentModules.length} modules</span>
                ...
                <div className={`${module.bgColor} divide-y divide-gray-100/80`}>
                  {module.videos.map((video, idx) => {
                    ...
                    return (
                      <button
                        key={video.id}
                        onClick={() => navigate(`/parent/video/${video.id}`)}

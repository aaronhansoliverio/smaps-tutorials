...
import { adminModules, getAdminTotalVideos } from '../data/courseData'
...
export default function AdminDashboard() {
  ...
  const [openModules, setOpenModules] = useState(['a1'])
  const [completedVideos, setCompletedVideos] = useState(() => {
    const saved = localStorage.getItem('smaps_admin_completed_videos')
    return saved ? JSON.parse(saved) : []
  })
  const [bannerError, setBannerError] = useState(false)

  const totalVideos = getAdminTotalVideos()
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <Navbar title="SMAPS-SIS Admin Tutorials" showBack backTo="/" backLabel="Home" />
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <h1 className="text-xl md:text-3xl font-extrabold leading-tight">
                SMAPS-SIS Admin Tutorials
              </h1>
              <p className="text-red-200 mt-2 text-sm leading-relaxed max-w-2xl">
                Complete video tutorial series for school administrators. Learn how to manage users,
                configure school settings, handle enrollment, oversee financials, and generate
                reports — all within SMAPS-SIS.
              </p>
              <div className="flex flex-wrap gap-3 mt-3 text-xs text-red-300">
                <span>📹 {totalVideos} videos</span>
                <span>📦 {adminModules.length} modules</span>
                ...
                <div className={`${module.bgColor} divide-y divide-gray-100/80`}>
                  {module.videos.map((video, idx) => {
                    ...
                    return (
                      <button
                        key={video.id}
                        onClick={() => navigate(`/admin-tutorials/video/${video.id}`)}
